...
chrome.runtime.onMessage.addListener(async (msg, sender, sendResponse) => {
    ...
    if(msg.type==="TAKE_BACK"){
      ...
      if (msg.type === "SEARCH_AND_PLAY") {
            ...
            (async () => {
                try {
                    const searchInput = await document.querySelector('input[type="search"][placeholder="Search"][data-testid="search-input__text-field"]');
                    searchInput.focus();
                    ...
                    const playbtn=document.querySelector('button.play-button.play-button--standard[data-testid="play-button"][aria-label="Play"]')
                    if(playbtn){
                        playbtn.click();
                    }
                } catch (err) {
